...
|---|---|---|---|---|
| **WIES Studio — dynamic spec sheets** | Spec sheets the brand websites generate on the fly for products that have **no** `specsheet_pdf` in Sales Layer | LARGELY RESOLVED 2026-07-22 via the crawl §E reconcile (approval gate closed): 138 dynamic spec-sheet URLs healed into `pdp_urls` (112 deduped rows) and folded into the KB by docs-ingest Step A. REMAINING TAIL: 757 unresolved + 1,370 collision PDPs (family model-code ambiguity, held for review per plan; one PPID = one page/sheet per Davis) — closes further as weekly crawls tighten matching, or fully if WIES exposes its URL table | Medium (was High) | Reconcile now runs clean (same-sku dedupe fix, PR #233); re-run `--reconcile-pdp --reconcile-write` after future crawls. |
