...
/** The dragged corners, in normalized image coords (0..1, y down; handles may
 * sit off-frame within -0.5..1.5). The front-TOP pair is optional for backward
 * compatibility (early calibrations derived the ceiling from the solve; they
 * are draggable now so the user can pin the ceiling seams — and they feed the
 * solver four extra edges). */
export interface RoomBoxCorners {
  backTopLeft: Pt;
  backTopRight: Pt;
  backBottomLeft: Pt;
  backBottomRight: Pt;
...
/** The eight box corners, for display. */
export type RoomBoxDisplayCornerId = RoomBoxCornerId;
...
/** The three orthogonal edge families generated by the corners. The optional
 * front-top pair adds the ceiling seams (depth) + the front verticals. */
export function boxAxisLines(corners: RoomBoxCorners): AxisLines[] {
  ...
  if (c.frontTopLeft && c.frontTopRight) {
    depth.push(
      { a: c.backTopLeft, b: c.frontTopLeft },
      { a: c.backTopRight, b: c.frontTopRight },
    );
    vertical.push(
      { a: c.frontTopLeft, b: c.frontBottomLeft },
      { a: c.frontTopRight, b: c.frontBottomRight },
    );
  }
  return [
    ...
    // World Y (depth): the floor seams (+ ceiling seams) running forward.
    { axis: "horizontalB", lines: depth },
    // World Z: the back wall's (+ front) vertical edges.
    { axis: "vertical", lines: vertical },
  ];
}
...
export function solveRoomBox(input: RoomBoxInput): RoomBoxSolved {
  ...
  // --- camera height: each vertical edge says its top ray (hits z=H) and
  // bottom ray (hits z=0) land at the same plan position → linear in cz. ------
  const vertPairs: Array<[Pt, Pt]> = [
    [c.backTopLeft, c.backBottomLeft],
    [c.backTopRight, c.backBottomRight],
  ];
  if (c.frontTopLeft) vertPairs.push([c.frontTopLeft, c.frontBottomLeft]);
  if (c.frontTopRight) vertPairs.push([c.frontTopRight, c.frontBottomRight]);
  const eqs: Array<{ d: number; h: number }> = [];
  for (const [top, bot] of vertPairs) {
    const rt = pixelRay(partial, top);
    const rb = pixelRay(partial, bot);
  ...
  const hit = (pt: Pt, planeZ: number): Vec3 | null => {
    const r = pixelRay(partial, pt);
  ...
  const bbl = hit(c.backBottomLeft, 0);
  const bbr = hit(c.backBottomRight, 0);
  const btl = hit(c.backTopLeft, H);
  const btr = hit(c.backTopRight, H);
  const fbl = hit(c.frontBottomLeft, 0);
  const fbr = hit(c.frontBottomRight, 0);
  if (!bbl || !bbr || !btl || !btr) {
  ...
  // Optional front-top (ceiling) corners refine the side walls + front extent.
  const ftl = c.frontTopLeft ? hit(c.frontTopLeft, H) : null;
  const ftr = c.frontTopRight ? hit(c.frontTopRight, H) : null;
  if ((c.frontTopLeft && !ftl) || (c.frontTopRight && !ftr)) {
    return fail("a ceiling corner points away from the ceiling — drag it up toward the ceiling line");
  }

  const yEst = [bbl.y, bbr.y, btl.y, btr.y];
  const yBack = yEst.reduce((s, v) => s + v, 0) / yEst.length;
  const xlEst = [bbl.x, btl.x, fbl.x, ...(ftl ? [ftl.x] : [])];
  const xrEst = [bbr.x, btr.x, fbr.x, ...(ftr ? [ftr.x] : [])];
  const xMin = xlEst.reduce((s, v) => s + v, 0) / xlEst.length;
  const xMax = xrEst.reduce((s, v) => s + v, 0) / xrEst.length;
  const yFront = Math.min(fbl.y, fbr.y, ...(ftl ? [ftl.y] : []), ...(ftr ? [ftr.y] : []));
 ...
 * ~55% of the frame, front floor/ceiling corners past the lower/upper image
 * corners. */
export function defaultRoomBoxCorners(): RoomBoxCorners {
  return {
    ...
    frontTopLeft: { x: -0.08, y: -0.06 },
    frontTopRight: { x: 1.08, y: -0.06 },
  };
}

/** Backfill the draggable front-top (ceiling) corners on a calibration saved
 * before they existed: place them where the SOLVE projects the ceiling
 * corners, so opening an old box doesn't move anything. A ceiling corner that
 * projects outside the draggable range is pulled back ALONG the seam in WORLD
 * space — and its floor corner moves to the same depth — so the side's two
 * front corners stay a true vertical pair (what the solver assumes). */
export function ensureTopCorners(
  corners: RoomBoxCorners,
  input: Omit<RoomBoxInput, "corners">,
): RoomBoxCorners {
  if (corners.frontTopLeft && corners.frontTopRight) return corners;
  const solved = solveRoomBox({ ...input, corners });
  const fallback = (): RoomBoxCorners => {
    const d = defaultRoomBoxCorners();
    return { ...corners, frontTopLeft: d.frontTopLeft, frontTopRight: d.frontTopRight };
  };
  if (!solved.ok) return fallback();

  const inRange = (p: Pt | null): p is Pt =>
    !!p && p.x >= -0.5 && p.x <= 1.5 && p.y >= -0.5 && p.y <= 1.5;
  const { xMin, xMax, yBack, yFront, height: H } = solved.box;
  const out: RoomBoxCorners = { ...corners };
  const sides = [
    { x: xMin, top: "frontTopLeft", bottom: "frontBottomLeft" },
    { x: xMax, top: "frontTopRight", bottom: "frontBottomRight" },
  ] as const;
  for (const side of sides) {
    const projTop = (y: number) => worldToImage(solved, { x: side.x, y, z: H });
    const atFront = projTop(yFront);
    if (inRange(atFront)) {
      out[side.top] = atFront;
      continue;
    }
    // Binary-search the seam depth where its projection enters the range
    // (the back-top corner end is always in range — the user dragged it).
    let lo = yFront;
    let hi = yBack;
    for (let i = 0; i < 24; i++) {
      const mid = (lo + hi) / 2;
      if (inRange(projTop(mid))) hi = mid;
      else lo = mid;
    }
    const yStar = Math.min(hi, yBack - 0.05);
    const top = projTop(yStar);
    const bottom = worldToImage(solved, { x: side.x, y: yStar, z: 0 });
    if (inRange(top) && bottom) {
      out[side.top] = top;
      out[side.bottom] = bottom; // same depth → the pair stays vertical
    } else {
      return fallback();
    }
  }
