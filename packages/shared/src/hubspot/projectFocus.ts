...
export function buildProjectFocusPrompt(input: {
  ...
  const system = [
    ...
    "",
    "RULES:",
    '- Default to ["Residential"] when the focus is unclear or only residential evidence exists.',
    '- Include "Commercial" ONLY when there is SPECIFIC evidence of commercial/contract work',
    "  (named hotels, restaurants, offices, retail, hospitality, institutional or multifamily projects).",
    "- A firm can do both — return both values when there is clear evidence of each.",
    "- Treat the website as a hint, not ground truth.",
    "",
